...
struct PaymentScheduleView: View {
    ...
    @State var actExpense: [ModelForGraph] = []
    ...
    var body: some View {
        NavigationStack {
            VStack {
                Text(calculationTotalExpenses(general: true))
                    .font(.caption)
                    .fontDesign(.serif)
                ...
                VStack(alignment: .leading, spacing: 20) {
                    VStack(spacing: 14) {
                        ...
                        Text(calculationTotalExpenses(general: false))
                            .font(.caption2)
    ...
    func AnimatedCharts() -> some View {
        let max = actExpense.max { item1, item2 in
            return item2.value > item1.value
        }?.value ?? 0
        
        Chart {
            ForEach(actExpense) { item in
                LineMark(
    ...
    func animateGraph(fromChange: Bool = false) {
        for (index, _) in actExpense.enumerated() {
...
private extension PaymentScheduleView {
...
struct PaymentScheduleView_Previews: PreviewProvider {
    static var previews: some View {
        PaymentScheduleView(expenses: [], title: "Test")
    }
}
